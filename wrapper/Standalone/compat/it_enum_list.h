...
// A shim, not a component of this app.
//
// gmpi_ui/experimental/builders.cpp - the widget set the settings page is built
// from - does `#include "it_enum_list.h"` and uses three of its functions to
// turn a combo box's comma-separated option string into entries. That header
// lives in the SynthEdit SDK (SynthEditLib/modules/shared/it_enum_list.h), so
// building those widgets normally requires SynthEditLib on the include path.
//
// GMPI_Wrappers depends on GMPI and gmpi_ui and nothing else, and that is worth
// keeping: a standalone app for a GMPI plugin should not drag in a DAW's source
// tree. So the three functions builders.cpp actually calls are reproduced here,
// byte-compatible with the original - the option-string syntax (`a,b,c`,
// explicit `name=7` ids, and `range lo,hi`) is a format plugins write, so the
// two parsers must agree exactly.
//
// Not on the public include path: only Standalone_Wrapper sees this directory,
// so a translation unit that legitimately wants the SynthEdit version still
// gets it.
//
// The `it_enum_list` CLASS from the original is deliberately absent. It needs a
// .cpp, and builders.cpp does not use it.
